...
GVOID_THRESHOLD = 10  # tries with zero hits → declare void tune 6‑10 if desired
MIN_BASE_COOLDOWN = 10
MAX_BASE_COOLDOWN = 60  # frames to stay disabled after first void 50 was original
...
# ─────────────────────────────────────────────────────────────────────────────
# >>>  original constant and class definitions remain unchanged …
#       (ellipsis here, paste your full class / helper definitions)
# ─────────────────────────────────────────────────────────────────────────────
# … [KEEP existing Antenna, helper functions, etc., unchanged] …
...
class Antenna:
    ...
    def choose_actionxxx(self, time_unit):
        ...
        if not action_pool:  # safety: never empty
            ...
            # The line above enables all the sectors!!!

            # The following is added to select sectors except the permanent cut sectors
            action_pool = [
                a
                for a in self.action_space
                if a not in self.disabled_actions  # PERMANENT CUT
        ...
        # after the first fallback works for GRIDS
        if not action_pool:  # still empty → last resort
            # 1) reopen one permanently-cut sector chosen at random
            a_reprobe = random.choice(list(self.disabled_actions))
            self.disabled_actions.remove(a_reprobe)  # re-enable it
            action_pool = [a_reprobe]  # now non-empty for sure
        ...
        if random.random() < self.epsilon:
            ...
            # print("Best ids:", best_idxs)
            best_act = action_pool[np.random.choice(best_idxs)]
    ...
    def choose_action(self, time_unit):
        ...
        action_pool = [
            a
            for a in self.action_space
            # if a not in self.disabled_actions]
            if time_unit >= self.disabled_until.get(a, -1)
        ]
        if not action_pool:  # safety: never empty
            action_pool = list(self.action_space)
    ...
    def update_Q(self, old_direction, action, reward, new_direction, time_unit):
        ...
        #         # -------------  void‑sector detection  -------------
        #         self.visits[action] += 1
        #         if reward > 0:                   # found a *new* neighbour
        #             self.hits[action] += 1
        #
        #         if (self.visits[action] >= self.VOID_THRESHOLD and self.hits[action] == 0):
        #             self.disabled_actions.add(action)
        #
        # ---------- local hit / cool‑down logic ----------
        # DEBUG
        # print("Time:", time_step, action)
        self.visits[action] += 1
        if reward > 0:
            self.hits[action] += 1
        ...
        if (self.visits[action] >= GVOID_THRESHOLD and self.hits[action] == 0) or (
            ...
            # V4
            # BASE_COOLDOWN = random.randint(MIN_BASE_COOLDOWN, MAX_BASE_COOLDOWN)
            # cooldown = BASE_COOLDOWN * (BACKOFF ** (count - 1))
            # self.disabled_until[action] = time_unit + cooldown  # ⟵ needs time!
...
try:
    ...
    random_flag = len(sys.argv) > 6 and sys.argv[6].lower() == "random"
