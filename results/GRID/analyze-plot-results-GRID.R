...
simPath <- "/my_nvme1/my_dir/my_courses/Aalborg/research/pub/NeighborDiscoveryDirectedAntennas/src/sim-v7/github/results/"
simPrefix <- "GRID"
...
file_map <- c(
  "Random Walk"          = "summary_rnd-wlk-grid.csv",
  "Plain QLearning"      = "summary_ql-grid.csv",
  "Adaptive ε"           = "summary_adaptive-grid.csv",
  "Pruning V1-1.1"              = "summary_pruning-v2-grid-backoff-1.1.csv",
  "Pruning V1-1.6"              = "summary_pruning-v2-grid-backoff-1.6.csv",
  "Pruning V2-1.1"              = "summary_pruning-v4-grid-backoff-1.1.csv",
  "Pruning V2-1.6"              = "summary_pruning-v4-grid-backoff-1.6.csv",
  "Adaptive ε + Pruning V1-1.6" = "summary_adaptive-pruning-v2-grid-backoff-1.6.csv",
  "Adaptive ε + Pruning V1-1.1" = "summary_adaptive-pruning-v2-grid-backoff-1.1.csv",
  "Adaptive ε + Pruning V2-1.6" = "summary_adaptive-pruning-v4-grid-backoff-1.6.csv",
  "Adaptive ε + Pruning V2-1.1" = "summary_adaptive-pruning-v4-grid-backoff-1.1.csv"
)
...
use_hatch     <- TRUE
legend_where  <- "top"
x_axis_label  <- "Grid dimension  N  (for an N×N lattice)"
...
for (metric_choice in metric_vec) {
  ...
  higher_is_betterx <- metric_choice %in% c("AVGEdgePercent", "EdgeExcessPercent", "EPTU")
  ...
  M_chr <- make_upper_triangle_delta(
    ...
    higher_is_better = higher_is_betterx,
    digits           = 3,
    diagonal         = "\\textemdash{}",
    show_sign        = FALSE
  )
...
plot_combo_scatter <- function(
    ...
    simPrefix     = get0("simPrefix", inherits = TRUE, ifnotfound = "GRID"),
...
plot_combo_scatter(raw_df, combo_order = c("A","G","AVGE"), simPrefix="GRID")
...
plot_combo_scatter(raw_df, combo_order = c("AVGE","G","A"), simPrefix="GRID")
...
plot_combo_scatter(raw_df, combo_order = c("AVGE","A","G"), simPrefix="GRID")
